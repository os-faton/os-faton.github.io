...
specialForms["если"] = function(args, env) {
	if (args.length != 3)
...
specialForms["цикл"] = function(args, env) {
	if (args.length != 2)
...
specialForms["сделать"] = function(args, env) {
	var value = false;
...
specialForms["опознать"] = function(args, env) {
...
topEnv["вывод"] = function(value) {
	document.getElementById("Terminal").value += value +"\n";
	return value;
};

specialForms["функция"] = function(args, env) {
	if (!args.length)
...
specialForms["изменить"] = function(args, env) {
	if (args.length != 2 || args[0].type != 'word')
	{
...
topEnv["массив"] = function() {
	var args = Array.prototype.slice.call(arguments, 0);
	return args;
};

topEnv["длина"] = function(array) {
	return array.length;
};

topEnv["элемент"] = function(array, n) {
	return array[n];
};
